...
export default function Logo({ size = 'md', showText = true, showTagline = false }: LogoProps) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      mainGradient.addColorStop(1, '#6366F1');
      ...
      // Draw "FS" letters
      ctx.fillStyle = 'white';
      ctx.font = `bold ${rectSize * 0.4}px Inter, system-ui, sans-serif`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.shadowColor = 'rgba(0, 0, 0, 0.2)';
      ctx.shadowBlur = 2;
      ctx.shadowOffsetY = 1;
      ctx.fillText('FS', centerX, centerY + 1);
  ...
  return (
    <Link href="/" className="flex items-center gap-3 group">
      ...
      >
        ...
        />
        <div
          className="absolute inset-0 rounded-xl bg-gradient-to-br from-emerald-500 via-blue-500 to-indigo-600 opacity-0 group-hover:opacity-100 blur-md transition-opacity duration-300"
          style={{ transform: 'scale(1.2)' }}
        />
      </div>
      {showText && (
        <div>
          <span className={`${currentSize.text} font-bold bg-gradient-to-r from-emerald-500 via-blue-500 to-indigo-600 bg-clip-text text-transparent`}>
            LearnFS
          </span>
          {showTagline && (
            <p className={`${currentSize.tagline} text-gray-500 dark:text-gray-400`}>
              Full-Stack in 6 Months
            </p>
          )}
        </div>
      )}
