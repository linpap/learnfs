...
export async function updateSession(request: NextRequest) {
  ...
  // Protected routes: Lesson 13 and above require authentication + premium
  // First 12 lessons (Month 1) are free
  const lessonMatch = url.pathname.match(/^\/lesson\/(\d+)$/)

  if (lessonMatch) {
    const lessonId = parseInt(lessonMatch[1], 10)
    const FREE_LESSONS = Array.from({ length: 12 }, (_, i) => i + 1) // Lessons 1-12
    ...
    if (!FREE_LESSONS.includes(lessonId)) {
      ...
      // Check premium status for lessons 13+
